...
import { pluginCodeFiles } from '../plugin-code';
...
export default function CodeExporter() {
  ...
  const handleDownloadZip = async () => {
    setZipping(true);
    try {
      // First attempt to download the pre-compiled, PclZip-validated ZIP
      try {
        const res = await fetch('/yasmine-artistry-booking.zip');
        if (res.ok) {
          const blob = await res.blob();
          const url = URL.createObjectURL(blob);
          const link = document.createElement("a");
          link.href = url;
          link.download = "yasmine-artistry-booking-v1.2.zip";
      ...
      <div id="file-tree-nav" className="lg:col-span-4 bg-white border border-slate-200/60 rounded-3xl p-5 shadow-xs flex flex-col justify-between">
        ...
        <div className="border-t border-slate-100 pt-4 mt-4 space-y-3">
          <button
            ...
            disabled={zipping}
            className="w-full flex items-center justify-center space-x-2 py-3 bg-indigo-600 hover:bg-indigo-750 text-white font-bold rounded-2xl transition duration-150 shadow-sm disabled:opacity-50 cursor-pointer text-xs"
          >
            {zipping ? (
              <>
              ...
              <>
                ...
                <span>Download Plugin v1.2 (.zip)</span>
              </>
            )}
          </button>
          <p className="text-[10px] text-slate-400 leading-normal text-center">
            Standard WordPress v1.2 archive ready to install on any self-hosted WordPress site.
          </p>
        </div>
      </div>
